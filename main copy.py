...
def getBattery():
    # Simulate battery voltage reading (replace with real OBD reading)
    if dev:
        return random.uniform(11, 15)
    adcpin = 3
    sensor = ADC(adcpin)
    adc_value = sensor.read_u16()
...
class SubieOBD(object):
    def __init__(self):
        ...
        self.pages = [self.page_temp, self.page_battery]
        self.page_index = 0
    ...
    def page_temp(self):
        ...
        if temp < 60:
            ...
            temp_color = (
                self.LCD.brown if hasattr(self.LCD, "brown") else self.LCD.yellow
            )  # High (use yellow or brown)
        else:
